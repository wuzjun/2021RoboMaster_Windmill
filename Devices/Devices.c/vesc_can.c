...
void vesc_can_set_duty(uint8_t controller_id, float duty) {
  ...
  uint8_t buffer[4];
  buffer_append_int32(buffer, (int32_t)(duty * 100000.0), &send_index);//数据类型转换
  sendPacket(&hcan1,controller_id | ((uint32_t)CAN_PACKET_SET_DUTY << 8), buffer, send_index);
}
...
//Helper methods
void sendPacket(CAN_HandleTypeDef* CANx,uint32_t id, uint8_t *data, int32_t len) {
  CAN_message_t  msg;
	if(len>8)
	{
		len = 8;
  ...
  msg.Can_TxHeader.ExtId = id;
  msg.Can_TxHeader.IDE = CAN_ID_EXT;
	msg.Can_TxHeader.RTR = 0;
	msg.Can_TxHeader.DLC =len;
  msg.timeout = 0;
	memcpy(msg.data, data, len * sizeof(uint8_t));
//  return (bool) CAN.write(msg);
	HAL_CAN_AddTxMessage(CANx,&msg.Can_TxHeader,data,(uint32_t*)CAN_TX_MAILBOX0);
}
